...
export class LAppWavFileHandler {
  ...
  public start(filePath: string): void {
    console.log("wav start", filePath);
    // サンプル位参照位置を初期化
    ...
    if (!this.loadWavFile(filePath)) {
      console.log("cannot load wav file", filePath);
      return;
    }
    console.log("load file success", filePath);
  }
  ...
  public loadWavFile(filePath: string): boolean {
    ...
    const asyncWavFileManager = (async () => {
      ...
      console.log( "file Byte", this._byteReader._fileByte );
      ...
      if (
        ...
        console.log( "false", this._byteReader._fileByte, this._byteReader._fileSize );
        return false;
      }
      ...
      try {
        ...
        if (!this._byteReader.getCheckSignature('RIFF')) {
          ret = false;
          console.log('Cannot find Signeture "RIFF".');
        ...
        if (!this._byteReader.getCheckSignature('WAVE')) {
          ret = false;
          console.log('Cannot find Signeture "WAVE".');
          throw new Error('Cannot find Signeture "WAVE".');
        }
        // シグネチャ "fmt "
        if (!this._byteReader.getCheckSignature('fmt ')) {
          ret = false;
          console.log('Cannot find Signeture "fmt".');
        ...
        if (this._byteReader.get16LittleEndian() != 1) {
          ret = false;
          console.log('File is not linear PCM.');
        ...
        if (this._byteReader._readOffset >= this._byteReader._fileSize) {
          ret = false;
          console.log('Cannot find "data" Chunk.');
        ...
        ret = true;
        console.log("load file ret1", ret);
      } catch (e) {
        console.log(e);
      }
    })();

    console.log("load file ret", ret);
    return ret;
  }
